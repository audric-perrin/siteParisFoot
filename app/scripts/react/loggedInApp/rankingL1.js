...
var RankingL1 = React.createClass({
  ...
  renderCell: function(element, specificStyle, logo) {
    ...
    var elements = []
    if (logo) {
      return d.td({style: style},
        React.createElement(Logo, {key: 'logo', name: element, float: 'left', margin: '2px 10px 0 0'}),
    ...
    if (this.state.ranking){
      for (var i = 0; i < this.state.ranking.length; i++) {
        if (i == 0) {
          ...
          elements.push(tr.apply(this, header))
        }
        elements.push(this.renderLineRanking(i))
      }
    }
    return d.table({}, d.tbody.bind(this, null).apply(this, elements))
  },
